...
The static `/admin/` application authenticates through a minimal GitHub App bridge.
GitHub owns the user identity, and the bridge verifies write access to `clopesle/artis`
before creating an administrative session.
...
The GitHub App client secret, session signing secret, and GitHub access tokens exist
only in the bridge environment. They must never be configured as `PUBLIC_*` variables or
...
configuration, approved site images, and `src/assets/catalog/`. Every write creates an
